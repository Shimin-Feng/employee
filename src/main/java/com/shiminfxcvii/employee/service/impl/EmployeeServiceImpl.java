...
import com.shiminfxcvii.employee.model.dto.EmployeeDTO;
...
import com.shiminfxcvii.employee.enums.Sex;
...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public void employee(Model model) {
        Page<Employee> employees = employeeRepository.findAll(PageRequest.of(Constants.ZERO_INTEGER, Constants.TEN_INTEGER,
                Sort.by(Constants.CREATED_DATE, Constants.EMPLOYEE_ID)));
    ...
    public ResponseEntity<String> saveOrUpdateEmployee(Principal user,
        ...
        for (int i = 0; i < bytes.length; i++) {
            sum += bytes[i] * Constants.WEIGHT[i];
        }
        if (bytes[18] != Constants.VALIDATE[sum % 11]) {
            // 设置响应头信息
            Constants.HTTP_HEADERS.set(CONTENT_TYPE, ALL_VALUE);

            return new ResponseEntity<>("身份证号码不正确", Constants.HTTP_HEADERS, BAD_REQUEST);
        }
        ...
        String dateTime = now.format(Constants.DATE_TIME_FORMATTER);
        ...
        if (HttpMethod.POST.equals(request.getMethod())) {
            ...
            if (employeeRepository.existsById(employee.getId())) {
                // 保存操作日志
                operationLogService.saveOperationLog(Constants.INSERT, employee, user);
                body = "添加成功。";
                status = OK;
            } else {
            ...
            if (employeeOptional.isPresent()) {
                ...
                if (Objects.equals(employee1.getEmployeeName(), cmd.getEmployeeName()) &&
                    ...
                    // 保存操作日志
                    operationLogService.saveOperationLog(Constants.UPDATE, employee1, user);
                    body = "修改成功。";
                    status = OK;
                } else {
        ...
        return new ResponseEntity<>(body, Constants.HTTP_HEADERS, status);
    }
        ...
        if (id != null) {
            ...
            if (employee.isPresent()) {
                ...
                if (employeeRepository.findById(id).isEmpty()) {
                    // 保存操作日志
                    operationLogService.saveOperationLog(Constants.DELETE, employee.get(), user);
                    body = "删除成功。";
                    status = OK;
                } else {
    ...
    public void findEmployeesBy(Integer pageNum,
        ...
        Page<Employee> employees = employeeRepository.findAll(
                ...
                PageRequest.of(pageNum, pageSize, Sort.by(direction, property, Constants.EMPLOYEE_ID))
        );
